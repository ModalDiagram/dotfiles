...
renderPlot({
  ...
  # we round the hours two the closest 10 min so that we can then group by
  # this periods and fill the missing ones
  selected.day.df$date <- round_date(selected.day.df$date, "10 min")
  ...
  ggplot(selected.day.df, aes(x=intervals)) +
    geom_bar(aes(weight=seconds), fill = "#287FD5") +
