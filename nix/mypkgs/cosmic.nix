...
in
{
  ...
  options = {
    services.desktopManager.cosmic_unstable = {
    ...
    environment.cosmic.excludePackages = lib.mkOption {
      ...
      default = [ ];
      example = lib.literalExpression "[ unstable_pkgs.cosmic-player ]";
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg = {
      ...
      portal = {
        enable = true;
        extraPortals = with unstable_pkgs; [
          xdg-desktop-portal-cosmic
          xdg-desktop-portal-gtk
        ];
        configPackages = lib.mkDefault [ unstable_pkgs.xdg-desktop-portal-cosmic ];
      };
    };

    systemd = {
      packages = [ unstable_pkgs.cosmic-session ];
      user.targets = {
    ...
    fonts.packages = with unstable_pkgs; [
      fira
      noto-fonts
      open-sans
    ];
    ...
    programs.dconf.packages = [ unstable_pkgs.cosmic-session ];
    security.polkit.enable = true;
    security.rtkit.enable = true;
    services.accounts-daemon.enable = true;
    services.displayManager.sessionPackages = [ unstable_pkgs.cosmic-session ];
    ...
    services.orca.enable = lib.mkDefault (notExcluded unstable_pkgs.orca);
